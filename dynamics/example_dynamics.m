...
num_timepoints = 200;         % Number of timepoints per subject
...
fii_timepoints = size(subs_fii, 2); %effective length of FII timepoints due to cumulative effect and derivative
%% Groupings
...
%%  Perform K-means
% Concatenate subjects and time of the subs_fii to get a 2D matrix
fii_k = reshape(subs_fii, num_subjects*fii_timepoints, num_features);
...
% Perform k-means
% [cluster_idx, cluster_c, cluster_sumd, cluster_D] = calculate_kmeans(fii_k, num_clusters, distance, replications, maxIter, useParallel, doDisplay);
% cluster_idx = reshape(cluster_idx, subjects_cnt, []); 

% Generate random cluster indices for fii to mimic kmeans index output
cluster_idx = randi([1 num_clusters], num_subjects * fii_timepoints, 1);

% Reshape into subjects x timepoints
cluster_idx = reshape(cluster_idx, num_subjects, fii_timepoints);

%% Extract groups from metrics
% Extract fii data for each group
fii_sz = subs_fii(sz_idx, :, :);
fii_cn = subs_fii(cn_idx, :, :);
...
sub_centroids_sz = group_sub_centroids(fii_sz, cluster_idx_sz, num_clusters);
